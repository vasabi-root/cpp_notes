...
Как это реализовать? [Crunch] Для функций T&& -- это универсальная ссылка.  Теперь типы у следующих выражений такие `¯\_( ͡° ͜ʖ ͡°)_/¯` :
```c++
int x;                    // int&
int &lrx = x;             // int&
...
Поэтому так важно писать `noxcept` в конструкторе, если он не бросает исключений
...
vec.pushback(vec[1]);
```
То есть, мы хотим скопировать существующий элемент вектора и положить эту копию в конец. Проблема здесь в том, что если сначала мувнуть, а потом обратиться к `vec[5]`, то будет UB!! Ведь мы уже забрали органы у старого вектора. Как быть? [Crunch] Сначала перемещаем vec[5] на новое место, а затем муваем `◑﹏◐`
